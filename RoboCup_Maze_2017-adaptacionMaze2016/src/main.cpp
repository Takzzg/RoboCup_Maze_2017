      ...
      //Serial.println(cant_puntos);
		...
		if (estado == est_calcular_dest) {
			...
			pc.printf("pos destino: %d, // ( %d, %d, %d)\n", masche.pos_destino,
					pnavegados[masche.pos_destino].X,
					pnavegados[masche.pos_destino].Y,
					pnavegados[masche.pos_destino].Z);
      //wait(2);
        ...
        /*for (int i = 0; i < abs(masche.giro); i++) {
          ...
          //wait_ms(25);
					...
					//DISTANCIA PARA ALEJARME O ACERCARME
					/*if (dist_n < 20) {
						while (dist_n != 6) {
							dist_n = ultra_n.distance();
							wait(0.1f);
